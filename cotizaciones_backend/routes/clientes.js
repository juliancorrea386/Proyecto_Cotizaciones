...
app.get('/', async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT id, nombre, telefono, municipio, created_at, updated_at FROM clientes ORDER BY nombre ASC');
    res.json(rows);
  } catch (err) {
    console.error(err);
...
app.get('/:id', async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT id, nombre, telefono, municipio FROM clientes WHERE id = ?', [req.params.id]);
...
app.post('/', async (req, res) => {
  try {
    ...
    await pool.query('INSERT INTO clientes (id, nombre, telefono, municipio) VALUES (?, ?, ?, ?)', [id, nombre, telefono, municipio]);
...
app.put('/:id', async (req, res) => {
  try {
    ...
    const result = await pool.query('UPDATE clientes SET nombre = ?, telefono = ?, municipio = ? WHERE id = ?', [nombre, telefono, municipio, req.params.id]);
